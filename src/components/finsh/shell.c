...
int finsh_getchar(void)
{
    ...
    {
        osSemaphoreAcquire(shell->rx_sem, osWaitForever);
...
static rt_err_t finsh_rx_ind(rt_device_t dev, rt_size_t size)
{
    ...
    osSemaphoreRelease(shell->rx_sem);
    return RT_EOK;
}
...
int finsh_system_init(void)
{
    ...
    const osThreadAttr_t shell_attr = {
        .name = FINSH_THREAD_NAME,
        .attr_bits = osThreadDetached,
        .cb_mem = NULL,
        .cb_size = 0,
        .stack_mem = NULL,
        .stack_size = FINSH_THREAD_STACK_SIZE,
        .priority = FINSH_THREAD_PRIORITY, // osPriorityBelowNormal
        .tz_module = 0,
        .reserved= 0
    };
    osThreadNew(finsh_thread_entry, NULL, &shell_attr);
    ...
    const osSemaphoreAttr_t rx_sem_attr = {
        .name = "shrx",
        .attr_bits = 0u,
        .cb_mem = NULL,
        .cb_size = 0u
    };
    shell->rx_sem = osSemaphoreNew(1, 0, &rx_sem_attr);

    finsh_set_prompt_mode(1);
    finsh_set_echo(FINSH_OPTION_ECHO);
