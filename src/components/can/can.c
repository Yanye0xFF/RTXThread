...
#define CAN_LOCK(can)   osMutexAcquire(can->lock, osWaitForever)
#define CAN_UNLOCK(can) osMutexRelease(can->lock)
...
rt_inline int _can_int_tx(struct rt_can_device *can, const struct rt_can_msg *data, int msgs) {
    int CAN_RETRY_MAX = can->config.sndboxnumber;
    int size, retry = 0;
    ...
    while (msgs) {
        ...
        osSemaphoreAcquire(tx_fifo->sem, osWaitForever);
        ...
        if ((can->ops->sendmsg(can, data, no) != RT_EOK) && (retry < CAN_RETRY_MAX)) {
            /* send failed. */
            rt_list_insert_after(&tx_fifo->freelist, &tx_tosnd->list);
            osSemaphoreRelease(tx_fifo->sem);
            osDelay(5);
            retry++;
            LOG_W("can send error !");
            continue;
        }

        if(retry >= CAN_RETRY_MAX) {
            can->status.dropedsndpkg++;
            // 还原所有占用的信号量
            osSemaphoreRelease(tx_fifo->sem);
            can->ops->control(can, RT_DEVICE_CTRL_CLR_INT, (void *)no);
            LOG_W("can retry timeout !");
            break;
        }
        retry = 0;
        can->status.sndchange = 1;
        status = osSemaphoreAcquire(tx_tosnd->completion, RT_CAN_SND_TIMEOUT);

        // 等待完成信号量时, tx_tosnd->result已被CAN中断修改

        result = (status == osOK) ? tx_tosnd->result : RT_CAN_SND_RESULT_ERR;
...
rt_inline int _can_int_tx_priv(struct rt_can_device *can, const struct rt_can_msg *data, int msgs)
{
    ...
    while (msgs)
    {
        ...
        if ((tx_fifo->buffer[no].result != RT_CAN_SND_RESULT_OK))
        {
            ...
            osSemaphoreAcquire(tx_fifo->buffer[no].completion, osWaitForever);
            continue;
        }
...
static rt_err_t rt_can_open(struct rt_device *dev, rt_uint16_t oflag)
{
    ...
    if (can->can_tx == RT_NULL)
    {
        if (oflag & RT_DEVICE_FLAG_INT_TX)
        {
            ...
            for (i = 0;  i < can->config.sndboxnumber; i++)
            {
                ...
                tx_fifo->buffer[i].completion = osSemaphoreNew(1, 0, NULL);
                tx_fifo->buffer[i].result = RT_CAN_SND_RESULT_OK;
            }

            rt_sprintf(tmpname, "%stl", dev->parent.name);
            sem_attr.name = tmpname;
            sem_attr.attr_bits = 0u;
            sem_attr.cb_mem = NULL;
            sem_attr.cb_size = 0u;
            tx_fifo->sem = osSemaphoreNew(can->config.sndboxnumber, can->config.sndboxnumber, &sem_attr);
    ...
    can->ops->control(can, RT_DEVICE_CTRL_OPEN, (void *)0);
...
static rt_err_t rt_can_close(struct rt_device *dev)
{
    ...
    can->ops->control(can, RT_DEVICE_CTRL_CLOSE, (void *)0);
                            ...
                            void                    *data)
{
    ...
    can->lock = osMutexNew(&can_mutex_attr);
...
void rt_hw_can_isr(struct rt_can_device *can, int event)
{
    ...
    case RT_CAN_EVENT_TX_FAIL:
    {
        ...
        osSemaphoreRelease(tx_fifo->buffer[no].completion);
        break;
    }
    }
}
