...
static int pwm_enable(int argc, char **argv)
{
    ...
    if (argc != 3)
    {
        rt_kprintf("Usage: pwm_enable <pwm_dev> <channel>\n");
        result = -RT_ERROR;
        goto _exit;
    }
    ...
    rt_kprintf("pwm enable result:%d\n", result);
_exit:
    return result;
}
MSH_CMD_EXPORT(pwm_enable, pwm_enable <pwm_dev> <channel>);
...
static int pwm_disable(int argc, char **argv)
{
    ...
    if (argc != 3)
    {
        rt_kprintf("Usage: pwm_disable <pwm_dev> <channel>\n");
        result = -RT_ERROR;
        goto _exit;
    }
...
MSH_CMD_EXPORT(pwm_disable, pwm_disable <pwm_dev> <channel>);
...
static int pwm_get(int argc, char **argv)
{
    ...
    if (argc != 3)
    {
        rt_kprintf("Usage: pwm_get pwm7 1\n");
