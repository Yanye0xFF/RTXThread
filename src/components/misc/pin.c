...
static rt_err_t _pin_control(rt_device_t dev, int cmd, void *args) {
    ...
    pin->ops->pin_mode(dev, (rt_base_t)mode->pin, (rt_base_t)mode->mode, mode->strength);
...
{
    RT_ASSERT(_hw_pin.ops != RT_NULL);
    _hw_pin.ops->pin_mode(&_hw_pin.parent, pin, mode, strength);
}
...
{
    RT_ASSERT(_hw_pin.ops != RT_NULL);
    return _hw_pin.ops->pin_toggle(&_hw_pin.parent, pin);
}
