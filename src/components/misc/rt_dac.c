...
/**
 * @brief DAC output voltage convert to DAC register value.
 * @param resolution if DAC is 8-bit or 12-bit resolution, fill 8 or 12.
 * @param output voltage range 0 ~ 3.3V.
 * @return DAC register value, right data alignment.
 * */
rt_uint32_t rt_dac_convert(rt_uint32_t resolution, float voltage) {
    rt_uint32_t value;
    float percent;

    percent = (voltage / 3.30f);
    value = (((rt_uint32_t)1 << resolution) - 1);
    percent *= (float)value;

    return (rt_uint32_t)percent;
}
