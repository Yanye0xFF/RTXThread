...
ALIGN(4) static osRtxMutex_t spi_bus_lock_entity;
                             ...
                             const struct rt_spi_ops *ops)
{
    ...
    osMutexAttr_t spi_bus_lock_attr;
    spi_bus_lock_attr.name = name;
    spi_bus_lock_attr.attr_bits = (osMutexRecursive|osMutexPrioInherit|osMutexRobust);
    spi_bus_lock_attr.cb_mem = (void *)&spi_bus_lock_entity;
    spi_bus_lock_attr.cb_size = sizeof(osRtxMutex_t);
    bus->lock = osMutexNew(&spi_bus_lock_attr);
...
rt_err_t rt_spi_configure(struct rt_spi_device        *device,
                          const struct rt_spi_configuration *cfg)
{
    osStatus_t status;
    ...
    {
        status = osMutexAcquire(device->bus->lock, osWaitForever);
        if (status == osOK)
        {
                               ...
                               rt_size_t             send_length2)
{
    ...
    rt_memset(&message, 0x00, sizeof(struct rt_spi_message));
