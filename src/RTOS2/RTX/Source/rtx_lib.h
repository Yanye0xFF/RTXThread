...
__STATIC_INLINE uint32_t osRtxMemoryInit (void *mem, uint32_t size) {
    (void)mem;
    (void)size;
    return 1U;
}
...
__STATIC_INLINE void    *osRtxMemoryAlloc(void *mem, uint32_t size, uint32_t type) {
    (void)mem;
    (void)type;
...
/// \return 1 - success, 0 - failure.
__STATIC_INLINE uint32_t osRtxMemoryFree (void *mem, void *block) {
    rt_free(block);
    return 1U;
}
